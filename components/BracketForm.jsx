...
export default function BracketForm() {
  ...
  return (
    <div className='bg-card border p-6 rounded-md max-w-[80%] mx-auto'>
      {!bracketCreated ? (
        <Form {...bracketForm}>
          <form onSubmit={bracketForm.handleSubmit(onBracketSubmit)} className='grid grid-cols-2 gap-8'>
            <FormField
        ...
        <Form {...teamForm}>
          <form onSubmit={teamForm.handleSubmit(onTeamSubmit)} className='space-y-6'>
            ...
            <p className="text-sm text-muted-foreground mb-4">Please enter at least 4 team names.</p>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder={`Team ${index + 1} Name`} />
                    </FormControl>
                  </FormItem>
                )}
              />
            ...
            <Button
              type="button"
              variant="outline"
              onClick={() => teamForm.setValue('teams', [...teamForm.watch('teams'), ''])}
            >
              Add Another Team
            </Button>
            <div>
              <Button type="submit" disabled={teamForm.formState.isSubmitting}>Submit Teams</Button>
            </div>
          </form>
        </Form>
      )}
